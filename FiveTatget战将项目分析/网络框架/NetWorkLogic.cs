...
public class NetWorkLogic
{
    ...
    public string GetIP() { //return m_strServerAddr; } //获取IP
    public int GetPort() { //return m_nServerPort; }  //获取端口

    public delegate void ConnectDelegate(bool bSuccess, string result);  //回调
    ...
    /// 设置回调
    /// </summary>
    /// <param name="delConnectFun"></param>
    ...
    void ConnectLost()
    {
        ...
        if (null != m_delConnectLost) m_delConnectLost();  //执行断线后的操作
    }
    ...
    void WaitPacket()
    {
        try
        {
            ...
            if (!ProcessInput())  //写入缓存区，不作处理
            {
                return;
            }
            if (!ProcessOutput())  //缓存器取出并发送
            {
                return;
            }
    ...
    /// 发送给服务器
    /// </summary>
    /// <param name="szServerAddr"></param>
    /// <param name="nServerPort"></param>
    /// <param name="nSleepTime"></param>
